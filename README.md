...
- **未分類フォルダをカテゴリ一覧の最上部へ固定**
  - 未分類はユーザー作成カテゴリではないため並び替え対象外
- **フォルダアイコンは視認性を優先して初期の `📁 / 📂` 表示を使用**
  - 4Kモニターでも判別しやすい表示を優先
- 左カラム下部のフォルダ操作説明文は非表示
...
- `src/dashboard-1.js` ～ `src/dashboard-12.js` : フィード取得・表示・重複判定・お気に入りカテゴリ・バックアップ・フォルダD&D・可変サイドバー・サイトアイコン取得・自動判定登録・RSS 1.0 RDF・フィードヘルスチェック・YouTubeチャンネル変換・カテゴリ管理分離・未分類固定などの処理

> MyFeed本体の拡張機能アイコンは検討中のため、v0.1.27では未実装です。
