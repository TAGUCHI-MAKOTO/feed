...
- 登録時も入力したWEBフィードが取得できなければ初回から旧RSS救済を実行
...
- `src/dashboard-1.js` ～ `src/dashboard-10.js` : フィード取得・表示・重複判定・お気に入りカテゴリ・バックアップ・フォルダD&D・可変サイドバー・サイトアイコン取得・自動判定登録・RSS 1.0 RDF・登録済みフィード自動修復などの処理

> MyFeed本体の拡張機能アイコンは検討中のため、v0.1.19では未実装です。